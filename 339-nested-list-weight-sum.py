...
# Output: 10
# Explanation: Four 1's at depth 2, one 2 at depth 1. 1*1 + 1*1 + 2*2 +
# 1*1 + 1*1 = 10.
...
class Solution:
    def getListSum(self, nestedList: List[NestedInteger], multiplier):
        res = 0
        for n in nestedList:
            if n.isInteger():
                res += multiplier * n.getInteger()
            else:
                res += self.getListSum(n.getList(), multiplier + 1)
        return res

    def depthSum(self, nestedList: List[NestedInteger]) -> int:
        return self.getListSum(nestedList, 1)
