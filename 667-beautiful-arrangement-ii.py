...
# contains n different positive integers ranging from 1 to n and obeys
# the following requirement:

# Suppose this list is [a1, a2, a3, ... , an], then the list [|a1 - a2|,
# |a2 - a3|, |a3 - a4|, ... , |an-1 - an|] has exactly k distinct
# integers.
...
class Solution:
    def constructArray(self, n: int, k: int) -> List[int]:
        left, right, res, zigzag = 2, n, [1], 0

        while k > 1:
            if zigzag:
                c, left, zigzag = left, left + 1, 0
            else:
                c, right, zigzag = right, right - 1, 1
            res.append(c)
            k -= 1

        rest = [i for i in range(left, right + 1)]
        if not len(res) % 2:
            rest.reverse()
        return res + rest
