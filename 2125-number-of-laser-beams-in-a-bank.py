...
# < r2. For each row i where r1 < i < r2, there are no security devices
# in the ith row. Laser beams are independent, i.e., one beam does not
# interfere nor join with another.
...
# on the 3rd row. This is because the 2nd row contains security devices,
...
class Solution:
    def numberOfBeams(self, bank: List[str]) -> int:
        last_level_lasers, beams = 0, 0
        for level in bank:
            lasers = 0
            for cell in level:
                lasers += 1 if cell == "1" else 0
            if lasers > 0:
                beams += last_level_lasers * lasers
                last_level_lasers = lasers
        return beams
