...
class Solution:
    def countVowelStrings(self, n: int) -> int:
        dp = [[1 if i == 0 else 0] * n for i in range(5)]
        for i in range(1, 5):
            for j in range(n):
                dp[i][j] = dp[i - 1][j] + 1 if j == 0 else dp[i - 1][j] + dp[i][j - 1]
        return dp[-1][-1]


t = Solution()
checkValue(5, t.countVowelStrings(1))
checkValue(15, t.countVowelStrings(2))
checkValue(66045, t.countVowelStrings(33))