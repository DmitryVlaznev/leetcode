...
class Solution:
    def slowestKey(self, releaseTimes: List[int], keysPressed: str) -> str:
        key, duration, prev = "a", float("-inf"), 0
        for time, candidate in zip(releaseTimes, keysPressed):
            if time - prev == duration:
                key = candidate if candidate > key else key
            elif time - prev > duration:
                key, duration = candidate, time - prev
            prev = time
        return key