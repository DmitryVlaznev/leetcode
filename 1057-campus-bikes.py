...
# is the position of the ith worker. You are also given an array bikes
# of length m where bikes[j] = [xj, yj] is the position of the jth bike.
# All the given positions are unique.

# Assign a bike to each worker. Among the available bikes and workers,
# we choose the (workeri, bikej) pair with the shortest Manhattan
# distance between each other and assign the bike to that worker.
...
# share the same distance to Bike 2, thus Worker 1 is assigned to Bike
    ...
    def assignBikes(
        ...
        for w in range(len(workers)):
            for b in range(len(bikes)):
                heapq.heappush(h, (dist(workers[w], bikes[b]), w, b))

        res, used_workers, used_bikes = [-1] * len(workers), set(), set()
        while h:
            _, w, b = heapq.heappop(h)
...
checkList([1, 0], s.assignBikes([[0, 0], [2, 1]], [[1, 2], [3, 3]]))
checkList([0, 2, 1], s.assignBikes([[0, 0], [1, 1], [2, 0]], [[1, 0], [2, 2], [2, 1]]))
