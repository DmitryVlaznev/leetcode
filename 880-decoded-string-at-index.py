...
# An encoded string S is given.  To find and write the decoded string to
# a tape, the encoded string is read one character at a time and the
...
class Solution:
    ...
    def decodeAtIndex(self, S: str, K: int) -> str:
        l = 0
        for c in S:
            l = l * int(c) if c.isdigit() else l + 1
...
checkValue("o", t.decodeAtIndex("leet2code3", 10))
checkValue("h", t.decodeAtIndex("ha22", 5))
checkValue("a", t.decodeAtIndex("a2345678999999999999999", 1))
checkValue("c", t.decodeAtIndex("abc", 3))