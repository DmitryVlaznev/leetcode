...
#   of insertion is min(2, 1) (elements 1 and 2 are less than 3, element
...
class Solution:
    def createSortedArray(self, instructions: List[int]) -> int:
        ...
        arr, res, mod = [], 0, 10 ** 9 + 7
        for n in instructions:
            l = bisect_left(arr, n)
            r = bisect_right(arr, n)
            if l <= len(arr) - r:
                res = (res + l) % mod
                arr[l:l] = [n]
            else:
                res = (res + len(arr) - r) % mod
                arr[r:r] = [n]
        return res


t = Solution()
checkValue(1, t.createSortedArray([1, 5, 6, 2]))
checkValue(3, t.createSortedArray([1, 2, 3, 6, 5, 4]))
checkValue(4, t.createSortedArray([1, 3, 3, 3, 2, 4, 2, 1, 2]))
