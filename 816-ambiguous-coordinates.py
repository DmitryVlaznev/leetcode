...
class Solution:
    def ambiguousCoordinates(self, s: str) -> List[str]:
        def canBeCoordinate(candidate: str):
            left, right = [], []
            cur = left
            for l in candidate:
                if l == ".":
            ...
            if len(left) > 1 and left[0] == "0":
                return False
            if len(right) > 0 and right[-1] == "0":
                return False
            return True

        def getCoordinates(digits):
            res = []
            if canBeCoordinate(digits):
                res.append(digits)
            for i in range(1, len(digits)):
                c = digits[0:i] + "." + digits[i:]
                if canBeCoordinate(c):
                    res.append(c)
            return res
        ...
        digits = s[1:-1]
        for i in range(1, len(digits)):
            first = getCoordinates(digits[0:i])
            second = getCoordinates(digits[i:])
...
print(t.ambiguousCoordinates("(0123)"))
