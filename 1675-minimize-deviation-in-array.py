...
# Input: nums = [1,2,3,4]
# Output: 1
# Explanation: You can transform the array to [1,2,3,2], then to
# [2,2,3,2], then the deviation will be 3 - 2 = 1.
...
class Solution:
    def minimumDeviation(self, nums: List[int]) -> int:
        ...
        heap, res = [], float("inf")
        for n in nums:
            heapq.heappush(heap, -1 * n if not n % 2 else -1 * 2 * n)
        smaller = max(heap)
        while not heap[0] % 2:
            m = heapq.heappop(heap)
            m //= 2
            smaller = max(smaller, m)
            heapq.heappush(heap, m)
            res = min(res, smaller - heap[0])
        return res


t = Solution()
print(t.minimumDeviation([3, 5]))
