...
# group of n monsters. You are given a 0-indexed integer array dist of
...
# you lose, or n if you can eliminate all the monsters before they reach
# the city.
...
class Solution:
    def eliminateMaximum(self, dist: List[int], speed: List[int]) -> int:
        ...
        for i in range(len(dist)):
            times[i] = dist[i] // speed[i] + (1 if dist[i] % speed[i] else 0)
        times.sort()
...
# checkValue(3, s.eliminateMaximum([1, 3, 4], [1, 1, 1]))
checkValue(3, s.eliminateMaximum([4, 2, 3], [2, 1, 1]))
