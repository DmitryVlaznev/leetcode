    ...
    def kthFactor(self, n: int, k: int) -> int:
        ...
        for i in range(1, int(n ** 0.5) + 1):
            if not n % i:
                ...
                complement = n // i
                if complement != i:
                    heapq.heappush(heap, -1 * complement)
                while len(heap) > k:
                    heapq.heappop(heap)
        if len(heap) == k:
            return -1 * heap[0]
...
checkValue(3, t.kthFactor(12, 3))
checkValue(12, t.kthFactor(12, 6))
checkValue(-1, t.kthFactor(12, 7))
checkValue(7, t.kthFactor(7, 2))
checkValue(-1, t.kthFactor(4, 4))
checkValue(1, t.kthFactor(1, 1))
checkValue(4, t.kthFactor(1000, 3))
