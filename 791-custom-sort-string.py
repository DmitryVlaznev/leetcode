...
# in order, so its position is flexible.
# Following the order of appearance in order, "b", "c", and "a" from s
# should be arranged as "b", "c", "a". "d" can be placed at any position
# since it's not in order. The output "bcad" correctly follows this
# rule. Other arrangements like "bacd" or "bcda" would also be valid, as
...
class Solution:
    def customSortString(self, order: str, s: str) -> str:
        ...
        od = defaultdict(lambda: float("inf"))
        ...
        def comparator(a, b):
            nonlocal od
            return -1 if od[a] < od[b] else 1 if od[a] > od[b] else 0

        return "".join(sorted(s, key=cmp_to_key(comparator)))
