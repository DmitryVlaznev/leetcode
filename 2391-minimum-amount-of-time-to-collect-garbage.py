...
# of metal, paper and glass garbage respectively. Picking up one unit of
...
# and must visit each house in order; however, they do not need to visit
# every house.
...
class Solution:
    def garbageCollection(self, garbage: List[str], travel: List[int]) -> int:
        ...
        for i, h in enumerate(garbage):
            ...
            res = res + gt + hg["G"] if hg["G"] > 0 else res
            gt = travel[i] if hg["G"] > 0 else gt + travel[i]

            res = res + pt + hg["P"] if hg["P"] > 0 else res
            pt = travel[i] if hg["P"] > 0 else pt + travel[i]

            res = res + mt + hg["M"] if hg["M"] > 0 else res
            mt = travel[i] if hg["M"] > 0 else mt + travel[i]
