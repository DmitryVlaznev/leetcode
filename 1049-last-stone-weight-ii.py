...
# possible weight of this stone (the weight is 0 if there are no stones
# left.)
...
class Solution:
    def lastStoneWeightII(self, stones: List[int]) -> int:
        ...
        maxSum = 0
        for i in stones:
            for j in range(targetSum - i, -1, -1):
                if dp[j]:
                    dp[j + i] = True
                    maxSum = max(maxSum, j + i)
        return totalSum - 2 * maxSum
t = Solution()

utils.checkValue(1, t.lastStoneWeightII([2,7,4,1,8,1]))
utils.checkValue(5, t.lastStoneWeightII([31,26,33,21,40]))