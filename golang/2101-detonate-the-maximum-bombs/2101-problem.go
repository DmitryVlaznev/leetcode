...
func maximumDetonation(bombs [][]int) int {
	var graph [][]int = make([][]int, len(bombs))
	for i, bi := range bombs {
		for j, bj := range bombs {
			xs, ys, rs, xd, yd := bi[0], bi[1], bi[2], bj[0], bj[1]
			if i != j && rs*rs >= (xd-xs)*(xd-xs)+(yd-ys)*(yd-ys) {
	...
	var max int = 0
	for i := range bombs {
		visited, q, imax := map[int]bool{}, []int{i}, 0
		visited[i] = true
		for len(q) > 0 {
			nq := []int{}
			for _, ibomb := range q {
				imax++
				neihgbors := graph[ibomb]
				for _, n := range neihgbors {
					if !visited[n] {
						nq = append(nq, n)
						visited[n] = true
					}
...
func main() {
	fmt.Println("..-== [2101. Detonate the Maximum Bombs] ==-..")

	fmt.Println(checkers.CheckScalar(1, maximumDetonation([][]int{{1, 1, 3}})))
	fmt.Println(checkers.CheckScalar(2, maximumDetonation([][]int{{2, 1, 3}, {6, 1, 4}})))
	fmt.Println(checkers.CheckScalar(2, maximumDetonation([][]int{{4, 4, 3}, {4, 4, 3}})))
	fmt.Println(checkers.CheckScalar(1, maximumDetonation([][]int{{1, 1, 5}, {10, 10, 5}})))
	fmt.Println(checkers.CheckScalar(5, maximumDetonation([][]int{{1, 2, 3}, {2, 3, 1}, {3, 4, 2}, {4, 5, 3}, {5, 6, 4}})))
}
