...
# top of the stack, they will take it and leave the queue. Otherwise,
# they will leave it and go to the queue's end. This continues until
# none of the queue students want to take the top sandwich and are thus
# unable to eat.

# You are given two integer arrays students and sandwiches where
# sandwiches[i] is the type of the i​​​​​​th sandwich in the stack (i =
# 0 is the top of the stack) and students[j] is the preference of the
...
class Solution:
    def countStudents(self, students: List[int], sandwiches: List[int]) -> int:
        dq = deque()
        waiting, eat, sandwich = 0, 0, 0
        for s in students:
            dq.append(s)

        while dq and waiting < len(dq):
            student = dq.popleft()
            if student == sandwiches[sandwich]:
                eat, sandwich, waiting = eat + 1, sandwich + 1, 0
            else:
                dq.append(student)
                waiting += 1
