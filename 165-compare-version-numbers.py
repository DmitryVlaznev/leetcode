...
class Solution:
    def compareVersion(self, version1: str, version2: str) -> int:
        version1 = [int(part) for part in version1.split(".")]
        version2 = [int(part) for part in version2.split(".")]
        last = max(len(version1), len(version2))
        for i in range(last):
            p1 = version1[i] if i < len(version1) else 0
            p2 = version2[i] if i < len(version2) else 0
            if p1 < p2:
                return -1
            if p1 > p2:
                return 1
