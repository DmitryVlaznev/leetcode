...
from utils import Node, graphFromArray
...
class Solution:
    def cloneGraph(self, node: "Node") -> "Node":
        ...
        while queue:
            cur, cur_neighbors = queue.pop()
            for n in cur_neighbors:
                if n.val in cloned:
                    cur.neighbors.append(cloned[n.val])
                else:
                    new_node = Node(n.val)
                    ...
                    cloned[n.val] = new_node
        return root


t = Solution()

t.cloneGraph(graphFromArray([[2, 4], [1, 3], [2, 4], [1, 3]]))
