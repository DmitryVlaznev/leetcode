...
# met. Any number of courses can be taken at the same time. Return the
# minimum number of months needed to complete all the courses.
...
# Thus, the minimum time needed to complete all the courses is 7 + 5 =
# 12 months.
    ...
    def minimumTime(self, n: int, relations: List[List[int]], time: List[int]) -> int:
        ...
        def tail_time(node):
            ...
            for next_node in graph[node]:
                tail = max(tail, tail_time(next_node))
            return time[node] + tail

        res = 0
        for i in range(n):
            res = max(res, tail_time(i))
