...
# 1 edges. The root of the tree is the node 0, and each node of the tree
...
# A subtree of the ith node which have the same label as node i.
...
# of its sub-tree. Node 1 has a label 'b'. The sub-tree of node 1
# contains nodes 1,4 and 5, as nodes 4 and 5 have different labels than
...
class Solution:
    def countSubTrees(self, n: int, edges: List[List[int]], labels: str) -> List[int]:
        ...
        def dfs(node_index, parent_index):
            ...
            if tree[node_index]:
                for child_index in tree[node_index]:
                    if child_index == parent_index:
                        continue
                    cl = dfs(child_index, node_index)
                    for label, num in cl.items():
                        l[label] += num
            l[labels[node_index]] += 1
            res[node_index] = l[labels[node_index]]
            return l
