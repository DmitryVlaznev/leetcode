...
class Solution:
    def numSpecial(self, mat: List[List[int]]) -> int:
        ...
        target_rows, target_cols = [set() for _ in range(rows)], [
            set() for _ in range(cols)
        ]

        for row in range(rows):
            for col in range(cols):
                if mat[row][col]:
                    target_rows[row].add(col)
                    target_cols[col].add(row)
        res = 0
        for row in range(rows):
            if len(target_rows[row]) == 1:
                col = next(iter(target_rows[row]))
                if len(target_cols[col]) == 1 and row in target_cols[col]:
                    res += 1
        return res
